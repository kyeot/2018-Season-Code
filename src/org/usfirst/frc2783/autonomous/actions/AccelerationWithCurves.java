...
public class AccelerationWithCurves extends Action {
	...
	/*The robot will accelerate as fast as it needs to, unless the calculated acceleration value is greater than the maximum
	 * acceleration. For all practical purposes, we use .5 percent per second per second.
	 */
	...
	/* Use this action when acceleration is not 0. All speeds refer to the speed of the robot in general, and are on a scale 
	 * from 0 to 1. To turn, pass through a non-zero radius. When referring to radius, use a positive number to represent a
	 * right turn, and a negative number to represent a left turn. As you may expect, when a radius is included, the motion
	 * followed by the robot will be in a circle with the specified radius.
	 * 
	 * In regards to how the action runs, given a starting speed, a max speed (desiredFinalSpeed), and an ending speed, 
	 * the robot obeys the following conditions:
	 ...
	 */
	public AccelerationWithCurves(double initX, double initY, double initialSpeed, double initialRadius, double finalX, double finalY,
			double desiredFinalSpeed, double radius, double nextX, double nextY, double nextFinalSpeed, double nextRadius, double time) {
		...
		if(initialSpeed >= desiredFinalSpeed) {
			scenario = nextFinalSpeed > desiredFinalSpeed ? "Both Bigger" : "Bigger then Smaller";
		} else {
			scenario = nextFinalSpeed > desiredFinalSpeed ? "Smaller then Bigger" : "Both Smaller";
		}
	}
	...
	public void perform() {
		...
		if(Math.abs(leftSpeed) > 1) {
			leftSpeed = 1;
			rightSpeed = (leftSpeed/(Math.abs(radius)+Constants.kTrackWidthInches))*(Math.abs(radius)-Constants.kTrackWidthInches);
		} else if (Math.abs(rightSpeed) > 1) {
			rightSpeed = 1;
			leftSpeed = (leftSpeed/(Math.abs(radius)+Constants.kTrackWidthInches))*(Math.abs(radius)-Constants.kTrackWidthInches);
		}
